# Typography
在一个站点中，文字的排版和样式应该是统一的。
我们将这部分的css写在layout.css 下面。

Typography中css，大部分都是Typographic。这部分css是会被继承的。
所以我们可以在lay模块中使用type 然后让其子元素继承这种页面排版的属性。

	
也需要排版的地方，在其父级元素添加 .typo  即可。
当只想用单独一个元素，或者让其显示和某个元素一样效果的时候，可以使用 .typo-TAGNAME 如 div.typo-small 可以获得 .typo small 一样的效果type.css。
